...
# Description: Bchat installation script
# OS: Ubuntu 18.04 LTS
# Script Version: 0.2
...
adduser --disabled-login --gecos "" bchat

sudo -i -u bchat bash << EOF
...
git clone https://github.com/chatwoot/bchat.git
cd bchat
if [[ -z "$1" ]]; then
  git checkout master;
else
...
echo "Woot! Woot!! Bchat server installation is complete"
echo "The server will be accessible at http://<server-ip>:3000"
echo "To configure a domain and SSL certificate, follow the guide at https://www.bchat.com/docs/deployment/deploy-bchat-in-linux-vm"
