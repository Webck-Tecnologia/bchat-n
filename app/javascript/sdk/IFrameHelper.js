...
export const IFrameHelper = {
  ...
  createFrame: ({ baseUrl, websiteToken }) => {
    ...
    iframe.id = 'bchat_live_chat_widget';
    iframe.style.visibility = 'hidden';

    let holderClassName = `woot-widget-holder woot--hide woot-elements--${window.$bchat.position}`;
    if (window.$bchat.hideMessageBubble) {
      holderClassName += ` woot-widget--without-bubble`;
    }
    if (isFlatWidgetStyle(window.$bchat.widgetStyle)) {
  ...
  getAppFrame: () => document.getElementById('bchat_live_chat_widget'),
  getBubbleHolder: () => document.getElementsByClassName('woot--bubble-holder'),
  sendMessage: (key, value) => {
    const element = IFrameHelper.getAppFrame();
    element.contentWindow.postMessage(
      `bchat-widget:${JSON.stringify({ event: key, ...value })}`,
      '*'
    );
  },
  initPostMessageCommunication: () => {
    window.onmessage = e => {
      if (
        ...
        e.data.indexOf('bchat-widget:') !== 0
      ) {
        return;
      }
      const message = JSON.parse(e.data.replace('bchat-widget:', ''));
  ...
  events: {
    loaded: message => {
      updateAuthCookie(message.config.authToken, window.$bchat.baseDomain);
      window.$bchat.hasLoaded = true;
      const campaignsSnoozedTill = Cookies.get('cw_snooze_campaigns_till');
      IFrameHelper.sendMessage('config-set', {
        locale: window.$bchat.locale,
        position: window.$bchat.position,
        hideMessageBubble: window.$bchat.hideMessageBubble,
        showPopoutButton: window.$bchat.showPopoutButton,
        widgetStyle: window.$bchat.widgetStyle,
        darkMode: window.$bchat.darkMode,
        showUnreadMessagesDialog: window.$bchat.showUnreadMessagesDialog,
      ...
      if (window.$bchat.user) {
        IFrameHelper.sendMessage('set-user', window.$bchat.user);
      }
      ...
      if (!window.$bchat.resetTriggered) {
    ...
    setBubbleLabel(message) {
      setBubbleText(window.$bchat.launcherTitle || message.label);
    },

    setAuthCookie({ data: { widgetAuthToken } }) {
      updateAuthCookie(widgetAuthToken, window.$bchat.baseDomain);
    },

    setCampaignReadOn() {
      updateCampaignReadStatus(window.$bchat.baseDomain);
    },
  ...
  onLoad: ({ widgetColor }) => {
    ...
    iframe.setAttribute('id', `bchat_live_chat_widget`);

    if (IFrameHelper.getBubbleHolder().length) {
      return;
    }
    createBubbleHolder(window.$bchat.hideMessageBubble);
    onLocationChangeListener();

    let className = 'woot-widget-bubble';
    let closeBtnClassName = `woot-elements--${window.$bchat.position} woot-widget-bubble woot--close woot--hide`;
