class Notification::PushNotificationService
  ...
  def perform
    ...
    notification_subscriptions.each do |subscription|
      ...
      send_push_via_bchat_hub(subscription)
    end
  end
  ...
  def send_browser_push(subscription)
    ...
    BchatExceptionTracker.new(e, account: notification.account).capture_exception
    true
  end
  ...
  def send_push_via_bchat_hub(subscription)
    return if firebase_credentials_present?
    return unless bchat_hub_enabled?
    return unless subscription.fcm?

    BchatHub.send_push(fcm_options(subscription))
  end
