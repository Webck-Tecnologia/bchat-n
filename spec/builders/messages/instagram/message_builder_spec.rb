...
describe Messages::Instagram::MessageBuilder do
  ...
  let!(:instagram_channel) { create(:channel_instagram_fb_page, account: account, instagram_id: 'bchat-app-user-id-1') }
  ...
  describe '#perform' do
      ...
      allow(fb_object).to receive(:get_object).and_return(
        {
          ...
          profile_pic: 'https://bchat-assets.local/sample.png'
    ...
    it 'discard echo message already sent by bchat' do
      message
    ...
    it 'creates message with for reply with story id' do
      ...
      expect(message.content_attributes[:story_id]).to eq('bchat-app-user-id-1')
      expect(message.content_attributes[:story_url]).to eq('https://bchat-assets.local/sample.png')
    end
